...
void UpdateTranslator::GetUpdatePR(noisepage::execution::compiler::FunctionBuilder *builder) const {
  ...
  if (op.GetChildrenSize() > 0) {
    ...
    for (const auto oid : all_oids_) {
      ...
      ast::Expr *set_pr =
          GetCodeGen()->PRSet(update_pr, col.Type(), col.Nullable(), idx, child_expr, true, col.MaxVarlenSize());
  ...
  for (const auto &clause : clauses) {
    ...
    auto *pr_set_call =
        GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(update_pr_), table_col.Type(), table_col.Nullable(),
                            table_pm_.find(table_col_oid)->second, clause_expr, true, table_col.MaxVarlenSize());
...
void UpdateTranslator::GenIndexInsert(WorkContext *context, FunctionBuilder *builder,
  ...
  for (const auto &index_col : index_schema.GetColumns()) {
    ...
    auto *set_key_call =
        GetCodeGen()->PRSet(index_pr_expr, attr_type, nullable, attr_offset, col_expr, true, index_col.MaxVarlenSize());
  ...
  for (const auto &index_col : index_cols) {
    ...
    auto *pr_set_call =
        GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(delete_index_pr), index_col.Type(), index_col.Nullable(),
                            index_pm.at(index_col.Oid()), val, true, index_col.MaxVarlenSize());
    builder->Append(GetCodeGen()->MakeStmt(pr_set_call));
  }
